...
/******************************************************************************
 *  A modified version of edu.princeton.cs.algs4.TST that allows using a
 *  backtracking traverser that only updates or checks the last character.
 ******************************************************************************/
...
public class TST<Value> {
	...
	public BacktrackTraverser<Value> getTraverser() {
		return new BacktrackTraverser<>(this);
	}
	 ...
	 * the character '.' is interpreted as a wildcard character.
	 * 
	...
	/**
	 * A traverser that allows jumping from node to node.
	 * 
	 * @author alfredaria2013
	 */
	public static class BacktrackTraverser<V> {

		TST<V> tst;
		Stack<Node<V>> nodes;

		private BacktrackTraverser(TST<V> tst) {
			this.tst = tst;
			nodes = new Stack<>();
		}
		...
		/**
		 * @param c
		 * @return true if there is a node that matches the character with the current
		 *         top of stack.
		 */
		public boolean hasDown(char c) {
			if (tst.root == null) {
				return false; // No root.
			}
			Node<V> cur = nodes.isEmpty() ? tst.root : nodes.peek().mid;
			while (cur != null) {
				if (cur.c == c) {
					return true;
				} else if (cur.c > c) {
		...
		/**
		 * If a child node is found that matches the character c, then push this node to
		 * the stack.
		 * 
		...
		public void down(char c) {
			...
			throw new IllegalArgumentException("Did not find proper node to append.");
		}

		/**
		 * Pop the backtracker stack a level up. Throw an illegalArgumentException if
		 * the stack size is already 0.
		 */
		public void up() {
			if (nodes.isEmpty()) {
		...
		/**
		 * @return True if the node at the top of the backtracking stack has a value.
		 *         This indicates that a word is found ending at this char.
		 */
		public boolean hasValue() {
			return !nodes.isEmpty() && nodes.peek().val != null;
		}
		
		public V getValue() {
			if (!hasValue()) {
				throw new IllegalArgumentException("No value at node.");
			}
			return nodes.peek().val;
		}

		/**
		 * @return The word represented by the current nodes stack.
		 */
...
/******************************************************************************
 * 
 * Liscense from the original class:
 *
